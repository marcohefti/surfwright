...
- Maintenance is explicit:
  - Reconcile/prune behavior should live in runtime commands (for example `state reconcile`, `target prune`, `session prune`) instead of hidden fix-ups on unrelated command paths.
