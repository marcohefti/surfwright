...
test("target upload keypress and drag-drop return deterministic shapes", () => {
  ...
  const html = [
    ...
    "source.addEventListener('dragstart',(e)=>e.dataTransfer.setData('text/plain','card-a'));",
    "target.addEventListener('dragover',(e)=>e.preventDefault());",
    "target.addEventListener('drop',(e)=>{e.preventDefault();document.getElementById('drop-result').textContent=e.dataTransfer.getData('text/plain');});",
    "</script>",
    "</main>",
  ].join("");
...
test("target fill and form-fill return deterministic compact shapes", () => {
  ...
  assert.deepEqual(Object.keys(fillPayload), [
    "ok", "sessionId", "sessionSource", "targetId", "actionId", "mode", "selector", "contains", "visibleOnly",
    "matchCount", "pickedIndex", "query", "valueLength", "url", "title", "wait", "timingMs",
  ]);
